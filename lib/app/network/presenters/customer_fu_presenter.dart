...
class CustomerFormUpdatePresenter {
  ...
  void fetchData(int id) async {
    Map data = {};
    try {
      Response customerReponse = await _bpCustomerService.show(id);
      Response customersResponse = await _getCustomers();
      Response typeResponse = await _getTypes();
      if (customersResponse.statusCode == 200 && typeResponse.statusCode == 200 && customerReponse.statusCode == 200) {
        data['customers'] = customersResponse.body;
        data['types'] = typeResponse.body;

        BpCustomer bpCustomer = BpCustomer.fromJson(customerReponse.body);
